...
autoplot(sngcll_pca,loadings = TRUE)
...
```
  ...
  Result from previously session is used as response variable and conduct classification. Lasso Multinomial model is selected for easy interpretation and variable selection. After model training, the first cluster has 27 gene signatures and the second cluster has 26 signatures, the third cluster has 33 gene signatures. A total of `r apply(sngcll_sign,2,FUN = function(x) gene[which(x !=0)]) %>% unlist %>% unique %>% length` unique gene is used as signature to distinguish groups.
  
  The first and second has `r ugene[[1]][ugene[[1]] %in% ugene[[2]]]` as common gene, and `r ugene[[1]][ugene[[1]] %in% ugene[[3]]] %>% str_c(collapse = ", ")` with the third cluster. `r ugene[[2]][ugene[[2]] %in% ugene[[3]]]%>% str_c(collapse = ", ")` are shared between second and the third cluster.
